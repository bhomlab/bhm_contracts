...
//
    ...
    function initialPayment(address _to, uint256 _keyTimestamp) public returns (bool){
        ...
        transfer(_to, initialPayStructs[_to][_keyTimestamp].deposit);
...
//    }
    ...
    function createAuction(uint256 _lowestprice, uint256 _agentFee, uint256 _auctionEndTime) public returns (uint256){
        ...
        auctionStructs[msg.sender][_keyTimeStamp].highestBid = auctionStructs[msg.sender][_keyTimeStamp].lowestprice;
        ...
        CreateAuction(msg.sender, _lowestprice, _agentFee, _keyTimeStamp, auctionStructs[msg.sender][_keyTimeStamp].auctionEndTime);
    }
    ...
    function bidAuction(address _beneficiary, uint256 _keyTimeStamp, uint256 _bid) public{
        ...
        require(auctionStructs[_beneficiary][_keyTimeStamp].lowestprice <= balanceOf(msg.sender));
        require(_bid <= balanceOf(msg.sender));
        ...
        // auctionStructs[_beneficiary][_keyTimeStamp].lock = true;
       	...
       	auctionStructs[_beneficiary][_keyTimeStamp].paymentTimestamp.push(now);
       	auctionStructs[_beneficiary][_keyTimeStamp].bidder.push(msg.sender);
        auctionStructs[_beneficiary][_keyTimeStamp].bid.push(_bid);
                                
        BidAuction(_beneficiary, now, msg.sender, _bid);
    } 
    ...
    function escroAuction(address _beneficiary, uint256 _keyTimeStamp) public onlyCertifiedAgent{
        ...
        require(auctionStructs[_beneficiary][_keyTimeStamp].agentFee <= balanceOf(_beneficiary));
        //require(auctionStructs[_beneficiary][_keyTimeStamp].auctionEndTime <= now);
        
        auctionStructs[_beneficiary][_keyTimeStamp].highestBid = auctionStructs[_beneficiary][_keyTimeStamp].bid[0];
        
        for(uint i = 0; i < auctionStructs[_beneficiary][_keyTimeStamp].bid.length; ++i){
            if(auctionStructs[_beneficiary][_keyTimeStamp].highestBid < auctionStructs[_beneficiary][_keyTimeStamp].bid[i]){
                auctionStructs[_beneficiary][_keyTimeStamp].highestBid = auctionStructs[_beneficiary][_keyTimeStamp].bid[i];
                auctionStructs[_beneficiary][_keyTimeStamp].highestBidder = auctionStructs[_beneficiary][_keyTimeStamp].bidder[i];
            }
        }
        setDeposit(auctionStructs[_beneficiary][_keyTimeStamp].highestBidder, _beneficiary, auctionStructs[_beneficiary][_keyTimeStamp].agentFee);
        setDeposit(_beneficiary, msg.sender, auctionStructs[_beneficiary][_keyTimeStamp].agentFee);
        
        withdrawDeposit(_beneficiary, msg.sender, auctionStructs[_beneficiary][_keyTimeStamp].agentFee);
        //auctionStructs[_beneficiary][_keyTimeStamp].lock = true;
...
//    function withDrawAuction(uint256 _keyTimeStamp) public{
...
//        
//        
//        //if(auctionStructs[msg.sender][_keyTimeStamp].highestBid < auctionStructs[_beneficiary][_keyTimeStamp].bid){
//            auctionStructs[msg.sender][_keyTimeStamp].highestBidder = auctionStructs[_beneficiary][_keyTimeStamp].bidder;
//            auctionStructs[msg.sender][_keyTimeStamp].highestBid = auctionStructs[_beneficiary][_keyTimeStamp].bid;
//            setDeposit(auctionStructs[msg.sender][_keyTimeStamp].highestBid , msg.sender, auctionStructs[msg.sender][_keyTimeStamp].highestBid);
//            // 최고가, 낙찰예정 확인 이벤트
////            HighestBidAuction(_beneficiary, now, auctionStructs[_beneficiary][_keyTimeStamp].highestBidder,
////         		auctionStructs[_beneficiary][_keyTimeStamp].highestBid);
//        //}
//        
//       
//        withdrawDeposit(auctionStructs[msg.sender][_keyTimeStamp].highestBidder, msg.sender, 
//            auctionStructs[msg.sender][_keyTimeStamp].highestBid);
//        auctionStructs[msg.sender][_keyTimeStamp].auctionEnded = true;
//        AuctionEnd(auctionStructs[msg.sender][_keyTimeStamp].highestBidder, auctionStructs[msg.sender][_keyTimeStamp].highestBid);
//    }

    // 5. Events
    event CreateAuction(address _beneficiary, uint256 _lowestprice, uint256 _agentFee, uint256 _keyTimeStamp, uint256 _auctionEndTime);

    event BidAuction(address _beneficiary, uint256 _keyTimeStamp, address bidder, uint256 _bid);

    event HighestBidAuction(address _beneficiary, uint256 _keyTimeStamp, address highestBidder, uint256 _highestBid);

    event EscroAuction(address _target, uint256 _keyTimeStamp);

    event AuctionEnd(address _highestBidder, uint256 _highestBid);

    event TestAuction(uint256 _keyTimeStamp, uint256 _highestBid);
    ...
    function withdrawLeaseFee(address _target, uint256 _keyTimeStamp) public{
        ...
        for(uint i = 0;
        i < leaseStructs[msg.sender][_keyTimeStamp].paymentTimestamp.length;
        ++i){
    ...
    function withdrawPreDeposit(address _to, uint256 _keyTimeStamp) public{
        // require ownership
        for(uint i = 0;
        i < leaseStructs[_to][_keyTimeStamp].paymentTimestamp.length;
        ++i){
